...
top:
	...
	/*
	 * Pageout only reads from these pages (their contents are copied out
	 * to the DMU via dmu_write()), so map the UPL range read-only.
	 *
	 * On Apple silicon, macOS 26 (Tahoe) enforces page-table permissions
	 * with the Secure Page Table Monitor (SPTM).  The legacy ubc_upl_map()
	 * always installs a writable kernel mapping (VM_PROT_DEFAULT ==
	 * READ|WRITE); when the underlying frame has been typed non-writable by
	 * SPTM -- as happens under heavy block-cloning (BRT) load -- requesting
	 * a writable mapping is an illegal transition and panics the machine:
	 *
	 *   [SPTM] VIOLATION_ILLEGAL_PERMS: sptm_map_page(...)
	 *       page_fte->type(0xe), new_perms(0x3) ...
	 *
	 * ubc_upl_map_range() (available since macOS 12) lets us map the range
	 * with VM_PROT_READ only, so SPTM never sees an illegal write request.
	 * It also returns the address already offset to upl_offset, so we no
	 * longer add upl_offset by hand.
	 */
...
#endif
{
	...
	while (isize > 0) {
		...
		if (!vaddr) {
			/*
			 * Map the pageout UPL read-only: we only read from it
			 * here (bluster_pageout() copies the data out via
			 * dmu_write()).  On Apple silicon under macOS 26 (Tahoe)
			 * the Secure Page Table Monitor panics if we request a
			 * writable kernel mapping over a frame it has typed
			 * non-writable -- which the legacy ubc_upl_map()
			 * (VM_PROT_DEFAULT == READ|WRITE) does, and which fires
			 * under heavy block-cloning load.  ubc_upl_map_range()
			 * (macOS 12+) lets us request VM_PROT_READ instead.
			 */
			if ((ubc_upl_map_range(upl, 0, a_size, VM_PROT_READ,
				    (vm_offset_t *)&vaddr) !=
	...
	// unmap
	if (vaddr) {
		ubc_upl_unmap_range(upl, 0, a_size);
		vaddr = NULL;
	}
out:
	zfs_rangelock_exit(lr);
